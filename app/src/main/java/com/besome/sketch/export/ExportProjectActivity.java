...
public class ExportProjectActivity extends BaseAppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        project_metadata = new ProjectWorkspace(getApplicationContext(), wq.d(sc_id), sc_metadata);
    ...
    private void exportSrc() {
        try {
            ...
            project_metadata.a(iCVar, hCVar, eCVar, ProjectWorkspace.ExportType.ANDROID_STUDIO);
    ...
    private void initializeAppBundleExportViews() {
        export_aab_button.setOnClickListener(view -> {
            ...
            confirmationDialog.setMessage("The generated .aab file must be signed.\nCopy your keystore to /Internal storage/lakiwebsites/keystore/release_key.jks and enter the alias' password.");
    ...
    private void showAabSigningDialog() {
        ...
        credentialsDialog.setListener(credentials -> {
            BuildingAsyncTask task = new BuildingAsyncTask(this, ProjectWorkspace.ExportType.AAB);
    ...
    private void initializeSignApkViews() {
        ...
        sign_apk_button.setOnClickListener(view -> {
            ...
            confirmationDialog.setMessage("""
                    ...
                    /Internal storage/lakiwebsites/keystore/release_key.jks and enter the alias's password.
    ...
    private void showApkSigningDialog() {
        ...
        credentialsDialog.setListener(credentials -> {
            ...
            BuildingAsyncTask task = new BuildingAsyncTask(this, ProjectWorkspace.ExportType.SIGN_APP);
            if (credentials != null) {
                if (credentials.isForSigningWithTestkey()) {
                    task.setSignWithTestkey(true);
                } else {
    ...
    private void initializeOutputDirectories() {
        signed_apk_postfix = File.separator + "lakiwebsites" + File.separator + "signed_apk";
        export_src_postfix = File.separator + "lakiwebsites" + File.separator + "export_src";
        /* /sdcard/lakiwebsites/signed_apk */
    ...
    private static class BuildingAsyncTask extends MA implements DialogInterface.OnCancelListener, BuildProgressReceiver {
        ...
        private final ProjectWorkspace project_metadata;
        private final WeakReference<LottieAnimationView> loading_sign_apk;
        private final ProjectWorkspace.ExportType exportType;
        ...
        public BuildingAsyncTask(ExportProjectActivity exportProjectActivity, ProjectWorkspace.ExportType exportType) {
        ...
        public void b() {
            ...
            try {
                ...
                oB oBVar = new oB();
                /* Check if /Internal storage/lakiwebsites/signed_apk/ exists */
                if (!oBVar.e(wq.o())) {
                ...
                if (buildingAppBundle) {
                    ...
                    builder.runZipalign(builder.ProjectWorkspace.unsignedUnalignedApkPath, builder.ProjectWorkspace.unsignedAlignedApkPath);
                    if (canceled) {
                        cancel(true);
                        return;
                    }
                    ...
                    String outputLocation = getCorrectResultFilename(builder.ProjectWorkspace.releaseApkPath);
                    if (signWithTestkey) {
                        TestkeySignBridge.signWithTestkey(builder.ProjectWorkspace.unsignedAlignedApkPath, outputLocation);
                    } else if (isResultJarSigningEnabled()) {
                        Security.addProvider(new BouncyCastleProvider());
                        CustomKeySigner.signZip(
                                new ZipSigner(),
                                ...
                                signingKeystorePassword,
                                signingAlgorithm,
                                builder.ProjectWorkspace.unsignedAlignedApkPath,
                                outputLocation
                        );
                    } else {
                        FileUtil.copyFile(builder.ProjectWorkspace.unsignedAlignedApkPath, outputLocation);
                    }
                }
        ...
        public void a() {
            ...
            if (buildingAppBundle && new File(Environment.getExternalStorageDirectory(), "lakiwebsites" + File.separator + "signed_aab" + File.separator + aabFilename).exists()) {
                ...
                dialog.setMessage("You can find the generated, signed AAB file at:\n" +
                        "/Internal storage/lakiwebsites/signed_aab/" + aabFilename);
