package laki.webide.blockSystem.extrablock;
...
public class ExtraPaletteBlock {
    ...
    public void setBlock(int paletteId, int paletteColor) {
        ...
        if (laki.webide.events.HTMLHead.EVENT_ID.equals(logicEditor.id)) {
            // Load only Web Blocks (Palette 88)
            ...
            for (HashMap<String, Object> map : arrayList) {
                if ("88".equals(map.get("palette"))) {
                    String type = (String) map.get("type");
                    String typeName = (String) map.get("typeName");
                    String name = (String) map.get("name");
                    logicEditor.a("", type, typeName, name);
                }
            }
            return;
        }

        if (laki.webide.events.ExtCSS.EVENT_ID.equals(logicEditor.id)) {
            // Load only CSS Blocks (Palette 89)
            logicEditor.a("Extend CSS", 0xFFFF9800);
            
            ArrayList<HashMap<String, Object>> arrayList = new ArrayList<>();
            ExtraBlocks.extraBlocks(arrayList);
            
            for (HashMap<String, Object> map : arrayList) {
                if ("89".equals(map.get("palette"))) {
                    String type = (String) map.get("type");
                    String typeName = (String) map.get("typeName");
                    String name = (String) map.get("name");
                    logicEditor.a("", type, typeName, name);
