...
import laki.webide.databinding.DialogSelectApkToSignBinding;
    ...
    private void signApkFileDialog() {
        ...
        apkPathDialog.setPositiveButton("Continue", (v, which) -> {
            ...
            String output_apk_path = new File(Environment.getExternalStorageDirectory(),
                    "lakiwebsites/signed_apk/" + output_apk_file_name).getAbsolutePath();
            ...
            if (new File(output_apk_path).exists()) {
                ...
                confirmOverwrite.setMessage("An APK named " + output_apk_file_name + " already exists at /lakiwebsites/signed_apk/.  Overwrite it?");
            ...
            public void run() {
                ...
                runOnUiThread(() -> {
                    if (ApkSigner.LogCallback.errorCount.get() == 0) {
                        ...
                        SketchwareUtil.toast("Successfully saved signed APK to: /Internal storage/lakiwebsites/signed_apk/"
